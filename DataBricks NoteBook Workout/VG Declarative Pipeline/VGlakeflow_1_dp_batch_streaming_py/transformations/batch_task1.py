...
@pipelines.table(name="catalog1_we47.schema1_we47.shipment1_bronze2")#it becomes declarative by specifying decorator on top of the function (Materialized view decorator is correct for batch ingestion)
def load_data_bronze_imp_dp():#Imperative program
    
    df1=spark.read.table("gcp_mysql_fc_we471.logistics.shipments1")#foreign catalog external DB source
