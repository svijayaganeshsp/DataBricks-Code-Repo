...
def drugstbl_bronze_tbl():
    return spark.read.table("catalog1_we47.schema_we47.druginfotbl")#requirement is to collect ins/upd/deleted data and merge to bronze table
...
def drugstbl_silvernewtbl():
    return spark.read.table("catalog1_we47.default.drugstbl_medal_bronze3").filter("rating IN (8,9,10)")
